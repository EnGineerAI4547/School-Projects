...
void psu_reduce(bigint256 r, const bigint512 a) {
    ...
    // Write 3 here
ADD_MUL_DIGIT_LOOP(a, (a+16), 38, r); 
r[0]=r[0]+(r[15]>>15)*19;

  
  r[15]&=0x7fff;
  CARRY_PROP_LOOP((r+0), r);
  SUB_LOOP(r, PRIME, r); 
  mask=0-(r[15]>>63);
  ADD_MASK_LOOP(r,mask, PRIME, r);
...
void mod_inv(bigint256 r, const bigint256 a) {
  ...
  /*
In binary, p − 2 is 250 1s, then 0, then 1, then 0, then 1, then 1.
*/
...
int main() {
    ...
    FILE *fbank = fopen("assign3b_bank","r");

    if (fbank == NULL) {
        printf("'assign3b_bank' file is missing. Please copy it to the same directory as where you execute the code.\n");
        return 1;
    }
